...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.max_epoch + 1):
        ...
        for i, batch in enumerate(train_loader, 1):
            ...
            results = dense_predict_network(feat_query, feat_shot, args)
            ...
            # loss = loss.mean()            
            loss = triplet_loss(results, label, thres=args.loss_gamma)
        ...
        avg_loss = avg_loss / i  # 计算平均损失
        # 使用writer记录平均损失和准确率
        writer.add_scalar('data/avg_loss_per_epoch', avg_loss, epoch)
        ...
        with torch.no_grad():
            for i, batch in enumerate(val_loader, 1):
                ...
                loss = triplet_loss(results, label, thres=0.2)
