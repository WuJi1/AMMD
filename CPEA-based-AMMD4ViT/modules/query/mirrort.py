...
# 动量更新support的encoder还没有实现
@registry.Query.register("MirrorT")
class ST(nn.Module):
    ...
    def forward(self, support_xf, support_y, query_xf, query_y):
        ...
        att = nn.Softmax(dim=-1)(simi_matrix)
        ...
        support_xf_center = support_xf.view(b, self.n_way, self.k_shot, c, h, w).permute(0,1,3,2,4,5).flatten(-3).mean(dim=-1)
        
        ##############################
        # support_xf_center = support_xf_center.unsqueeze(1).unsqueeze(-2).expand_as(aligned_query_support)
        # support_xf_center = _l2norm(support_xf_center, dim=-1)
        # aligned_query_support = _l2norm(aligned_query_support, dim=-1)
        # dist_token2center = torch.norm(support_xf_center - aligned_query_support, dim=-1) ** 2 
        # similarity = -dist_token2center.mean(dim=-1).view(b*q, self.n_way)

        # import ipdb 
        # ipdb.set_trace()
        ##########################
        support_xf_center = support_xf_center.unsqueeze(1).unsqueeze(-2).expand(-1, q, -1, -1, -1)
        support_xf_center = _l2norm(support_xf_center, dim=-1)
        aligned_query_support = _l2norm(aligned_query_support, dim=-1)
        similarity = aligned_query_support @ support_xf_center.transpose(-1, -2)
        similarity = similarity.flatten(-2).mean(dim=-1).view(b*q, self.n_way)
