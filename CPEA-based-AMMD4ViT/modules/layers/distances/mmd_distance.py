...
class MMDDistance(nn.Module):
    ...
    def compute_mmd(kernel_ss, kernel_qq, kernel_sq, beta, gamma, tem):
        ...
        # top_k = 5
        # filter_value = 0
        # indices_to_remove = kernel_sq < torch.topk(kernel_sq, top_k)[0][..., -1, None]
        # kernel_sq[indices_to_remove] = filter_value
        #
        # indices_to_remove = kernel_qq > torch.topk(kernel_qq, top_k)[0][..., -1, None]
        # kernel_qq[indices_to_remove] = filter_value
        #
        # indices_to_remove = kernel_ss > torch.topk(kernel_ss, top_k)[0][..., -1, None]
        # kernel_ss[indices_to_remove] = filter_value
        ...
        if beta is None:
            ...
            # mmd_s1 = beta.transpose(-1, -2) @ kernel_ss @ beta
        ...
        if gamma is None:
            ...
            # diag_sum = (gamma.squeeze(-1) ** 2 * kernel_qq_).sum(-1)
            ...
            # mmd_sq = 1.0 / (num_feat_s * num_feat_q) * mmd_sq
        # mmd_dis = mmd_s + mmd_q - 2.0 * mmd_sq
