...
class MMD(nn.Module):
    def __init__(self, in_channels=384, loss="ce", kernel="linear"):
        ...
        self.mmd = MMDDistance(kernel=self.kernel)
        # else:
    ...
    def inference(self, support_xf, query_xf, beta=None, gamma=None):
        ...
        # if self.cfg.model.mmd.switch == "all_supports":
            # if self.num_groups > 1:
                ...
                #                                                           self.group_dim)
            # else:
            support_xf = support_xf.reshape(b, self.n_way, -1, nf, c).permute(0, 2, 1, 3, 4)
...
class AttentiveMMDPrompt(MMD):
    def __init__(self, args, in_channels=384, loss="ce", kernel="linear"):
        super().__init__(in_channels, loss, kernel)
        self.feat_dim = 384
    ...
    def forward(self,  query_xf, support_xf, args):
        # support_xf = support_xf.unsqueeze(0)
        # query_xf = support_xf.unsqueeze(0)
        ...
        #####
        support_xf = support_xf.unsqueeze(0).unsqueeze(-2)
        query_xf = query_xf.unsqueeze(0).unsqueeze(-2)
        b, s, c, h, w = support_xf.shape
        q = query_xf.shape[1]
        fhw, fc = support_xf.shape[-2:]
        
        support_xf = support_xf.permute(0, 1, 4, 2, 3)
        query_xf = query_xf.permute(0, 1, 4, 2, 3)      
        ...
        return self.inference(support_xf, query_xf, beta, gamma)
