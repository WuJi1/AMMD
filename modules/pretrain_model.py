...
class PretrainModel(nn.Module):
    def __init__(self, cfg, fea_size=5):
        super().__init__()
        ...
        self.avg_pool = nn.AvgPool2d(fea_size, stride=1)
    ...
    def forward_proto(self, support_x, support_y, query_x, query_y):
        ...
        rewards = [1 if predict_labels[j]==query_y[j].to(predict_labels.device) else 0 for j in range(len(query_y))]
        return rewards
    def forward(self, x=None, y=None, support_x=None, support_y=None, query_x=None, query_y=None):
        if self.training:
            return self.forward_train(x, y)
        else:
            return self.forward_proto(support_x, support_y, query_x, query_y)


class PretrainVit(PretrainModel):
    def __init__(self, cfg, fea_size=5):
        super().__init__(cfg, fea_size)
        if cfg.vit_aug.mixup > 0.:
            self.criterion = SoftTargetCrossEntropy()
        elif cfg.vit_aug.label_smoothing > 0.:
            self.criterion = LabelSmoothingCrossEntropy(smoothing=cfg.vit_aug.label_smoothing)
        else:
            self.criterion = nn.CrossEntropyLoss()
...
def make_pretrain_vit(cfg):
    return PretrainVit(cfg, fea_size=cfg.pre_vit.win_size)